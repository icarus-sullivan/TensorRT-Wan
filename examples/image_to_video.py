"""Image-to-video: the same `WanEngine.generate()` call as text-to-video, plus an `image=`
first-frame tensor. See docs/python_api.md — I2V/T2V share one code path by design (PLAN.md's
"one unified engine, not one per workflow" principle).
"""
...
def load_first_frame(path: str, device: torch.device) -> torch.Tensor:
...
def main() -> None:
    ...
    first_frame = load_first_frame("first_frame.png", engine.device)
